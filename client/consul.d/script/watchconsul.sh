...
wifi_setup() {
  if (nc -vz 8.8.8.8 53 >/dev/null 2>&1) \
    && [ $(cat /sys/class/net/${dev}/operstate) == "up" ] \
    && (/sbin/ifconfig $dev | grep "inet addr" >/dev/null 2>&1); then
    :
  else
    /sbin/ifdown ${dev} && /sbin/ifup ${dev} && sleep 5 && status="${status}|ifrestart ${dev}"
  fi
}
